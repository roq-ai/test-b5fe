...
type ColumnType = ColumnDef<TrainingProgramInterface, unknown>;

interface TrainingProgramListPageProps {
...
export function TrainingProgramListPage(props: TrainingProgramListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getTrainingPrograms({
        relations: ['habit', 'progress.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TrainingProgramInterface>>(
    () => `/training-programs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TrainingProgramInterface) => {
    if (hasAccess('training_program', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/training-programs/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'name', header: 'Name', accessorKey: 'name' },
    hasAccess('habit', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'habit',
          header: 'Habit',
          accessorKey: 'habit',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/habits/view/${record.habit?.id}`}>
              {record.habit?.name}
            </Link>
          ),
        }
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="training_program" />
        </Flex>
